...
const user1 = createUser("Alex", "alex@email.com", 25);
const user2 = createUser("Jordan", "jordan@email.com", 30, true);

console.log(user1);
console.log(user2);

// isPremium = false is a good default because most users will probably start as regular users unless they choose premium.
...
// isValidUser is a function expression because we're storing the function inside a const variable.
...
// An arrow function is a good fit here because the function is short and only returns one value.
...
console.log(findUserById(users, 3));  // should find Zoe
console.log(findUserById(users, 99)); // null
...
console.log("Adults:", adults.map(user => user.username));

console.log("Young adults count:", youngAdults.length);
console.log("Young adults:", youngAdults.map(user => user.username));

console.log("Teens count:", teens.length);
console.log("Teens:", teens.map(user => user.username));
// ----------------------------------------------------------
...
const accountStats = getAccountStats(users);

console.log(accountStats);

// Passing isValidUser without () gives filter the function so filter can use it on each user.
// isValidUser() would run the function right away instead.
// ----------------------------------------------------------
...
// Changing user.isPremium also changes the original object because objects are passed by reference,
// so user and sam are pointing to the same object.
// ----------------------------------------------------------
...
// We use u[field] because field is a variable that holds the property name.
// u.field would look for a property literally named "field".