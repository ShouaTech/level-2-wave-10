...
for (let i = 0; i < inventory.length; i++) {
    console.log(`# ${inventory[i].price} | Stock: ${inventory[i].stock} | ${inventory[i].isAvailable}`);
}
...
let found = 0;
for (let i = 0; i < inventory.length; i++) {
    if (searchCategory === inventory[i].category) {
        found += 1;
        console.log(`🔍 ${inventory[i].name} price $${inventory[i].price}`);
    }
}
...
for (let i = 0; i < inventory.length; i++) {
inventory[i].supplier = {name: "Amazon", country: "China"};
}
...
console.log(`🏆 Most valuable: ${inventory[topValueIndex].name} | $${inventory[topValueIndex].price} x ${inventory[topValueIndex].stock} units = $${(inventory[topValueIndex].price * inventory[topValueIndex].stock)}`);
...
for (let i = 0; i < categories.length; i++) {
    let numberOfProducts = categories.length;
    let totalStock;
    for (let i = 0; i < inventory.length; i++) {
